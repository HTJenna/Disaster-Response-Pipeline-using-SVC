...
def build_model():
    ...
    """
    # model pipeline
    SVC_pipeline = Pipeline([('vect', CountVectorizer(tokenizer=tokenize)),
                         ('tfidf', TfidfTransformer()),
                         ('clf', MultiOutputClassifier(
                            OneVsRestClassifier(LinearSVC())))])

    # hyper-parameter grid
    SVC_parameters = {'vect__ngram_range': ((1, 1), (1, 2)),
                  'vect__max_df': (0.75, 1.0)
                  }

    # creating the  model
    return GridSearchCV(estimator=SVC_pipeline,param_grid=SVC_parameters,verbose=3,cv=2)
